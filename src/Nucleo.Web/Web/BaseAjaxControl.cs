...
{
	/// <summary>
	/// Represents the base class for AJAX controls.
	/// </summary>
	/// <example>
	/// [
	...
	/// </example>
	[
	WebScriptMetadata(typeof(BaseAjaxControlScriptMetadata))
	]
	public abstract class BaseAjaxControl : WebControl, IScriptControl, IPostBackDataHandler, IAjaxControl, IClientStateControl, IAjaxScriptableComponent, IRenderableControl, IControlDriver
	{
		...
		/// </summary>
		[ClientProperty("clientStateField", ContentType = ClientPropertyContentType.ClientElement)]
		...
		/// <summary>
		/// Gets or sets whether client-state is enabled for controls.
		/// </summary>
		public virtual bool EnableClientState
		{
		...
		/// </summary>
		[ClientProperty("enabled", ContentType = ClientPropertyContentType.Data)]
		...
		protected virtual void GetAjaxScriptDescriptors(IContentRegistrar registrar)
		{
			...
			ClientDescriptorRegistry registry = ClientDescriptorRegistry.CreateFor(this);

			if (registry != null)
			{
				foreach (ClientDescriptorEvent eventInfo in registry.Events)
				{
					descriptor.AddEventIfExists(eventInfo.ClientName,
						(string)Reflect.Public(this).Property(eventInfo.ClientEventHandlerPropertyName ?? eventInfo.ServerName).GetValue());
				}

				foreach (ClientDescriptorProperty propertyInfo in registry.Properties)
				{
					if (propertyInfo.Value is IScriptComponent)
					{
						IScriptComponent comp = (IScriptComponent)propertyInfo.Value;
						comp.RegisterAjaxDescriptors(registrar, descriptor);
						comp.RegisterAjaxReferences(registrar);
						continue;
					}

					if (propertyInfo.ContentType == ClientPropertyContentType.ClientElement)
						descriptor.AddElementPropertyIfExists(propertyInfo.ClientName, (string)propertyInfo.Value);
					else if (propertyInfo.ContentType == ClientPropertyContentType.AjaxComponent)
						descriptor.AddComponentPropertyIfExists(propertyInfo.ClientName, (string)propertyInfo.Value);
					else
						descriptor.AddPropertyIfExists(propertyInfo.ClientName, propertyInfo.Value);
				}
			}
			else
			{
				...
				descriptor.AddPropertyIfExists("enabled", this.Enabled);
		...
		protected virtual void GetCssReferences(IContentRegistrar registrar)
		{
			...
			IWebSingletonManager singletons = WebSingletonManager.GetCurrent();
			if (singletons == null)
				return;

			//Check the AJAX manager for enabled theming
			NucleoAjaxManager manager = singletons.GetEntry<NucleoAjaxManager>();
			if (manager != null && manager.EnableCustomTheming)
				return;
		...
		IEnumerable<ScriptReference> IScriptControl.GetScriptReferences()
		{
			...
			NucleoAjaxManager manager = WebSingletonManager.GetCurrent().GetEntry<NucleoAjaxManager>();
			if (manager == null)
				throw new ArgumentNullException("manager", "The NucleoAjaxManager instance could not be found.");
		...
		{
			if ((Page is IPageDriver && !((IPageDriver)Page).IsInitialLoad) || Page.IsPostBack)
			{
		...
		protected override void OnInit(EventArgs e)
		{
			...
			if (context != null && context.Singletons != null)
			{
				...
				StylesheetManager styles = context.Singletons.GetEntry<StylesheetManager>();
				if (styles != null)
				{
					if (this.EnableCustomTheming)
						styles.AddComponent(this);
				}
			}
		...
		protected override void OnPreRender(EventArgs e)
		{
			...
			if (this.RegisterWithScriptManager)
				this.GetScriptManager(this.Page).RegisterScriptControl<BaseAjaxControl>(this);
