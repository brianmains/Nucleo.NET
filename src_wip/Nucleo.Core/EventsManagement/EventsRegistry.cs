...
namespace Nucleo.EventsManagement
{
	...
	public class EventsRegistry : IDisposable
	{
		...
		private EventRegistryOptions _options = null;
		private List<PublishedEventDetails> _publishings = null;
		...
		/// <summary>
		/// Gets the options to use for the registry.
		/// </summary>
		public EventRegistryOptions Options
		{
			get 
			{
				if (_options == null)
				{
					_options = new EventRegistryOptions { QueuePublishedNotifications = false };
		...
		public void Publish(PublishedEventDetails publishedDetails)
		{
			...
			this.QueuePublishing(publishedDetails);
		}
			...
			if (this.Items.Count > 0)
			{
				for (int index = 0, len = this.Items.Count; index < len; index++)
				{
					...
					foreach (PublishedEventDetails publishDetails in publishedDetails)
					{
						...
						this.QueuePublishing(publishDetails);
					}
				}
			}
			else
			{
				foreach (var publishDetails in publishedDetails)
					this.QueuePublishing(publishDetails);
			}
		}
		...
		private void QueuePublishing(PublishedEventDetails publishedDetails)
		{

			//Not null check simple enough to determine if queuing of publishing is enabled
			if (_publishings == null)
				return;

			_publishings.Add(publishedDetails);
		}
		...
		public void Subscribe(IListenerCriteria criteria, IEventSubscriber receiver)
		{
			...
			this.SubscribeToDetails(new SubscriptionEventDetails(criteria, receiver));
		}
		...
		public void Subscribe(IListenerCriteria criteria, Action<PublishedEventDetails> action)
		{
			...
			this.SubscribeToDetails(new SubscriptionEventDetails(criteria, action));
		}
		...
		public void Subscribe(SubscriptionEventDetails details)
		{
			...
			this.SubscribeToDetails(details);
		}
		...
		public void Subscribe(IEnumerable<SubscriptionEventDetails> details)
		{
			...
			foreach (SubscriptionEventDetails detail in details)
				this.SubscribeToDetails(detail);
		}
		...
		private void SubscribeToDetails(SubscriptionEventDetails details)
		{
			this.Items.Add(details);

			if (_publishings != null)
			{
				var publishedFindings = _publishings.Where(i => details.Criteria.IsMatch(i.Criteria)).ToList();
				foreach (var publishedFinding in publishedFindings)
				{
					this.PublishDetails(details, publishedFinding);
				}
			}
		}
		...
		{
			if (this.Options.QueuePublishedNotifications)
			{
				if (_publishings == null)
					_publishings = new List<PublishedEventDetails>();
			}
			else
			{
				if (_publishings != null)
