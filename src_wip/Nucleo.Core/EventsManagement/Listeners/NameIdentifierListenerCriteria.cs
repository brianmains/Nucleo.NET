	...
	public class NameIdentifierListenerCriteria : IListenerCriteria
	{
		...
		public virtual bool IsMatch(IListenerCriteria criteria)
		{
			if (criteria is NameIdentifierListenerCriteria)
				return (string.Compare(this.Identifier, ((NameIdentifierListenerCriteria)criteria).Identifier, StringComparison.OrdinalIgnoreCase) == 0);
			else if (criteria is EventListenerCriteria)
				return (string.Compare(this.Identifier, ((EventListenerCriteria)criteria).Event.Name, StringComparison.OrdinalIgnoreCase) == 0);
		...
		{
			return "Identifier set to " + Identifier;
		}
