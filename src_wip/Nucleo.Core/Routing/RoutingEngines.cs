...
namespace Nucleo.Routing
{
	...
	public class RoutingEngines
	{
		...
		/// <summary>
		/// Gets or sets the handler of failures.
		/// </summary>
		public IRoutingFailureHandler FailureHandler
		{
			get;
			set;
		}
		...
		public void Navigate(object routingSource)
		{
			...
			if (engine is IRoutingSecurity)
			{
				var security = (IRoutingSecurity)engine;
				if (!security.HasPermission(routingSource))
				{
					if (this.FailureHandler != null)
						this.FailureHandler.HandleFailure(engine, routingSource, RoutingFailureReason.Security);
			...
			{
				if (this.FailureHandler != null)
					this.FailureHandler.HandleFailure(engine, routingSource, RoutingFailureReason.Error);
			}
		}
